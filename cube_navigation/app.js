...
Cube.prototype.sideChange = function() {
  ...
  let all_no_select = document.getElementsByClassName("noselect");
  for(let i = 0; i < all_no_select.length; i++) {
      if(all_no_select[i].style.animation != undefined) {all_no_select[i].style.animation = "none"}
    
  }
  let home_page_class = document.getElementsByClassName('home-page')
  for(var i = 0; i < home_page_class.length; i++) {
       (home_page_class[i].style.opacity = "0")
  }
  let changed_face = (this.sides[this.viewport.currentSide - 1].classList[1]);
  (document.getElementsByClassName(`home-page ${changed_face}`)[0].style.opacity = "1");
  let noselect_class = (document.getElementsByClassName(`home-page ${changed_face}`)[0].querySelectorAll(".noselect"))
  for(var i = 0; i < noselect_class.length; i++) {
     noselect_class[i].style.animation = "opacity-anim 2s"
  }
